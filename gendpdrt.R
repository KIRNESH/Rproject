...
newton <- function(f1,f2,tol = 1e-7 , x0 = rep(1,2), N = 1000){
  ...
  while (i <= N) {
    ...
    if (norm(abs(x1 - x0),type="2") < tol || (f1value<1e-8 && f2value<1e-8 )) break
    x0 = x1
  }
  return(p)
}

gendpdrt=function(smp,alp,beta,x0){
  ...
  fqint=function(p){
    if(p[2]>=0){
      ...
      try(integrate(function(x){(dnorm(x,mean=p[1],sd=p[2])^(1+alp))*(((x-p[1])^2/(p[2]^2)-1)/p[2])*exp(beta*dnorm(x,mean=p[1],sd=p[2]))},p[1]-40*p[2],p[1]+40*p[2],rel.tol=1e-7)$value,silent=T)}
    else{
