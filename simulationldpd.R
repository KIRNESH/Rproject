...
simulationldpd=function(){
  ...
  while(i<=500){
    print(i)
    smp=rnorm(1000,0,1)
    #cat(smp,file="mutstcheck20.txt",sep=",",append=TRUE)
    #cat("\n",file="mutstcheck20.txt",append=TRUE)
    #r=median(smp)
    ...
    est0[[i]]=ldpd(smp,0,st[[index]][1:2])
    for(j in 2:(length(valuef))){
      if(est0[[i]]=="Start a new solution") {
    ...
    st=checkldpd(smp,0.02)
    valuef=sapply(st,function(x) x[3])
    valuesort=sort(valuef)
    index=which(valuef==valuesort[1])
    est0.02[[i]]=ldpd(smp,0.02,st[[index]][1:2])
    for(j in 2:length(valuef)){
      if(est0.02[[i]]=="Start a new solution") {
    ...
    st=checkldpd(smp,0.05)
    valuef=sapply(st,function(x) x[3])
    valuesort=sort(valuef)
    index=which(valuef==valuesort[1])
    est0.05[[i]]=ldpd(smp,0.05,st[[index]][1:2])
    for(j in 2:length(valuef)){
      if(est0.05[[i]]=="Start a new solution") {
        index = which(valuef == valuesort[j])
        est0.05[[i]]=ldpd(smp,0.05,st[[index]][1:2])
      }
    ...
    st=checkldpd(smp,0.1)
    valuef=sapply(st,function(x) x[3])
    valuesort=sort(valuef)
    index=which(valuef==valuesort[1])
    est0.1[[i]]=ldpd(smp,0.1,st[[index]][1:2])
    for(j in 2:length(valuef)){
      if(est0.1[[i]]=="Start a new solution") {
        index = which(valuef == valuesort[j])
        est0.1[[i]]=ldpd(smp,0.1,st[[index]][1:2])
      }
    ...
    st=checkldpd(smp,0.25)
    valuef=sapply(st,function(x) x[3])
    valuesort=sort(valuef)
    index=which(valuef==valuesort[1])
    est0.25[[i]]=ldpd(smp,0.25,st[[index]][1:2])
    for(j in 2:length(valuef)){
      if(est0.25[[i]]=="Start a new solution") {
        index = which(valuef == valuesort[j])
        est0.25[[i]]=ldpd(smp,0.25,st[[index]][1:2])
      }
      else if (abs(est0.25[[i]][1]) > 1) {
        index = which(valuef == valuesort[j])
        est0.25[[i]]=ldpd(smp,0.25,st[[index]][1:2])
      }
    ...
    st=checkldpd(smp,0.5)
    valuef=sapply(st,function(x) x[3])
    valuesort=sort(valuef)
    index=which(valuef==valuesort[1])
    est0.5[[i]]=ldpd(smp,0.5,st[[index]][1:2])
    for(j in 2:length(valuef)){
      if(est0.5[[i]]=="Start a new solution") {
        index = which(valuef == valuesort[j])
        est0.5[[i]]=ldpd(smp,0.5,st[[index]][1:2])
      }
      else if (abs(est0.5[[i]][1]) > 1) {
        index = which(valuef == valuesort[j])
        est0.5[[i]]=ldpd(smp,0.5,st[[index]][1:2])
      }
    ...
    valuesort=sort(valuef)
    index=which(valuef==valuesort[1])
    est1[[i]]=ldpd(smp,1,st[[index]][1:2])
    for(j in 2:length(valuef)){
      if(est1[[i]]=="Start a new solution") {
        index = which(valuef == valuesort[j])
        est1[[i]]=ldpd(smp,1,st[[index]][1:2])
      }
      else if (abs(est1[[i]][1]) > 1) {
        index = which(valuef == valuesort[j])
        est1[[i]]=ldpd(smp,1,st[[index]][1:2])
      }
    ...
    cat(i,est0[[i]][1],est0.02[[i]][1],est0.05[[i]][1],est0.1[[i]][1]
        ...
        file="mutstldpd1000.txt",sep="\t",append=TRUE)
    
    cat("\n",file="mutstldpd1000.txt",append=TRUE)
    i=i+1
  }
  ...
  write.xlsx(dmu,file="n1000.xlsx",sheetName="Sheet1ldpd",append=T)
  write.xlsx(dsigma,file="n1000.xlsx",sheetName="Sheet2ldpd",append=T)
  cat("\n\nRelative Effeciency for n=1000",file="effeciencyldpd1000.txt",sep="\n",append=T)
  cat("\nAlpha               relefmu            relefsigma",file="effeciencyldpd1000.txt",append=TRUE)
  relefmu=c()
  relefsigma=c()
  ...
  for(i in 1:7){
    cat("\nAlpha=",x[i],"       ",relefmu[i],"        ",relefsigma[i],file="effeciencyldpd1000.txt",append=TRUE)
  }
}