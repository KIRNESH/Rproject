...
gendpder=function(smp,alp,beta,x0){
  ...
  fqint=function(p){
    if(p[2]>=0){
      try(integrate(function(x){(dnorm(x,mean=p[1],sd=p[2])^(1+alp))*(((x-p[1])^2/(p[2]^2)-1)/p[2])*exp(beta*dnorm(x,mean=p[1],sd=p[2]))},-Inf,Inf)$value,silent=T)}
    else{
      #return("Start with new solution")}
  }}
