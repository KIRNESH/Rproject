...
gaussgendpd=function(smp,alp,beta,x0){
  
  v=seq(0,2000,1)
  
  
  fmd=function(p){if(p[2]>=0){
    sum((dnorm(smp,mean=p[1],sd=p[2])^alp)*(smp)*exp(beta*dnorm(smp,mean=p[1],sd=p[2])))/length(smp)}
    else{
  ...
  fsd=function(p){if(p[2]>=0){sum((dnorm(smp,mean=p[1],sd=p[2])^alp)*((smp-rep(p[1],length(smp)))^2)*exp(beta*dnorm(smp,mean=p[1],sd=p[2])))/length(smp)}
    else{return("Start with new solution")}}
  
  fqint=function(p){
    if(p[2]>=0){
      
      try(integrate(function(x){(dnorm(x,mean=p[1],sd=p[2])^(1+alp))*((x-p[1])^2)*exp(beta*dnorm(x,mean=p[1],sd=p[2]))},-Inf,Inf,rel.tol=1e-7)$value,silent=T)}
    else{
      return("Start with new solution")}
  }
  if(class(fqint)=="try-error"){return("Start with new solution")}
  fsigma=function(p){try(sqrt(abs((fsd(p)-fqint(p))/(fwd(p)-fmint(p)))),silent=T)}
